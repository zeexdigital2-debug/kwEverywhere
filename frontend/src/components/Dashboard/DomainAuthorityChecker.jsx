...
import { Shield, Search, Loader2, Globe, Award, BarChart3, ExternalLink } from 'lucide-react';
...
export default function DomainAuthorityChecker() {
  ...
  const handleCheck = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to fetch domain metrics');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="glass-panel p-6 rounded-3xl border border-gray-800 shadow-xl">
      ...
      <div className="flex items-center space-x-3 mb-6 border-b border-gray-800 pb-4">
        ...
        <div>
          <h3 className="text-lg font-bold text-white">Domain Authority & PageRank Checker</h3>
      ...
      {result && (
        <div className="bg-dark-800/80 p-6 rounded-2xl border border-gray-700/80 animate-in fade-in zoom-in-95">
